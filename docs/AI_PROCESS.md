...
Numa segunda rodada montei a ParceirosPage.vue consumindo os componentes do design system e o composable. Nessa etapa, o Gemini gerou um template inicial com uma cor de texto errada nas informações da listagem, o conteúdo ficava praticamente invisível na tela por falta de contraste. Precisei corrigir manualmente, padronizando as classes de cor pra garantir que as informações da tabela ficassem legíveis, o que também reforça a consistência visual pedida no requisito de padronização.

Por fim, após testes manuais de acessibilidade, identifiquei que o formulário permitia submit com campos obrigatórios vazios, retornando erro cru do Postgres na tela. Corrigi adicionando validação client-side com Zod antes do envio. Também identifiquei que o KSelect não tinha indicador de foco visível ao navegar por teclado, a causa era o uso de `focus:` numa classe aplicada à raiz de um componente PrimeVue, que não é o elemento que recebe o foco de verdade; troquei para `focus-within:`, que resolveu. Configurei Storybook do zero, incluindo o registro do PrimeVue com o mesmo tema da aplicação no `preview.ts`, já que sem isso os componentes renderizavam sem estilo. Por fim, escrevi um teste unitário para o tratamento do erro de CNPJ duplicado (código 23505) em `useParceiros`, usando Vitest com mock do Supabase.
